...
import { CSM_DISPLAY_NAMES, ACTIVITIES, ACTIVITY_CATEGORIES, validateSubmission, calcPoints } from "../types/index.js";
...
export default function SubmissionForm({ onSubmit, editTarget, onCancelEdit, disabled }) {
  ...
  useEffect(() => {
    if (editTarget) {
      setForm({
        csm:           editTarget.displayName || editTarget.csm || "",
  ...
  if (act?.showContext && !form.context?.trim()) missing.push(act.contextLabel || "Context");
      ...
      <Card>
        ...
        <FormField label="CSM Name *" error={fe("csm")}>
          <select value={form.csm} onChange={e => set("csm", e.target.value)} style={inp(touched.csm && !form.csm)}>
            ...
            {CSM_DISPLAY_NAMES.map(n => <option key={n}>{n}</option>)}
          </select>
        </FormField>

        {/* Activity — grouped by category */}
          ...
          <>
            <FormField label="Customer Name *" error={fe("customerName")}>
              <input type="text" value={form.customerName}
                ...
                style={inp(touched.customerName && !form.customerName?.trim())} />
            </FormField>
        ...
        {act?.showContext && (
          <FormField label={`${act.contextLabel} *`} error={fe("context")}>
            <input type="text" value={form.context}
              ...
              placeholder={act.contextPlaceholder}
              style={inp(touched.context && !form.context?.trim())} />
          </FormField>
        )}
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            {missing.map(m => (
              <span key={m} style={{ fontSize: 12, color: colors.muted, background: "#f7f6f2",
                border: `1px solid ${colors.border}`, padding: "3px 9px", borderRadius: 20 }}>
                • {m}
              </span>
            ))}
          </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={save} disabled={saving || disabled || !ready}
            style={{
              flex: 1, border: "none", borderRadius: 10, fontSize: 14, fontWeight: 600,
              padding: "12px 20px",
              cursor: (saving || disabled || !ready) ? "not-allowed" : "pointer",
          ...
          {isEditing && (
            <button onClick={cancel} disabled={saving}
              style={{ border: `1px solid ${colors.border}`, borderRadius: 10, fontSize: 14,
      ...
      {!isEditing && (
        <div style={{ marginTop: 20, padding: "18px 20px", background: "#fff",
          ...
          <div style={{ display: "grid", gap: 5 }}>
            ...
            <div><strong style={{ color: colors.dark }}>3.</strong> Fill in the required fields — they change based on activity type.</div>
            <div><strong style={{ color: colors.dark }}>4.</strong> Submit unlocks when all required fields are valid.</div>
          </div>
