import { canonicalCsmName, normalizeQuarter } from '../types/index.js';
...
export async function fetchSubmissions() {
  ...
  // Defensive normalization: resolves legacy CSM aliases to their canonical
  // name and classifies any row without a Quarter tag (pre-dating that
  // column) as Q2 2026, regardless of whether the Apps Script backend has
  // been redeployed with the same logic yet.
  return data.submissions.map(row => ({
    ...row,
    csm:     canonicalCsmName(row.csm),
