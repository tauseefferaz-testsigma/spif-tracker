import {
  currentWeekNumber, PROGRAM_WEEKS, getPaceStatus, CURRENT_QUARTER,
} from "../types/index.js";
...
export function buildSlackMessage(submissions, quarter = CURRENT_QUARTER) {
  const week    = currentWeekNumber();
  const summary = buildTeamSummary(submissions, quarter);
  const lb      = buildCsmStats(submissions, quarter);
  ...
  return `📊 Customer Advocacy App — Snapshot | ${quarter} · ${dateStr} · Week ${week} of ${PROGRAM_WEEKS}
...
export function buildConsolidatedSlackMessage(submissions, quarter = CURRENT_QUARTER) {
  const week    = currentWeekNumber();
  const stats   = buildCsmStats(submissions, quarter);
  const today   = new Date();
  const dateStr = today.toLocaleDateString("en-US", { month: "short", day: "numeric" });

  let message = `🏆 Customer Advocacy Leaderboard — ${quarter}\n`;
