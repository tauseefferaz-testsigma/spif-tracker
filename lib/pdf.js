...
import { formatDate, CURRENT_QUARTER } from '../types/index.js';
import { buildLeaderboard, buildActivityBreakdown, buildTeamSummary } from './stats.js';

export function exportReport(submissions, quarter = CURRENT_QUARTER) {
  const doc      = new jsPDF();
  const today    = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
  const stats    = buildTeamSummary(submissions, quarter);
  const lb       = buildLeaderboard(submissions, quarter);
  ...
  doc.text(`Customer Advocacy App — Report (${quarter})`, 14, 22);
  ...
  doc.save(`advocacy-report-${quarter.replace(/\s+/g, '-').toLowerCase()}-${new Date().toISOString().split('T')[0]}.pdf`);
}
