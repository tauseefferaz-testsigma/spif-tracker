...
import { QUARTERS, CURRENT_QUARTER, normalizeQuarter } from './types/index.js';
...
export default function App() {
  ...
  // Scope every view (dashboard, leaderboard, log) to the selected quarter.
  // Q2 2026 data is never touched — this only changes which slice of it is
  // displayed. Rows created before the Quarter column existed are treated
  // as Q2 2026 (see normalizeQuarter).
  const quarterSubmissions = useMemo(
    () => submissions.filter(r => normalizeQuarter(r.quarter) === quarter),
    [submissions, quarter]
  );
    ...
    <div style={{ minHeight: '100vh', background: colors.bg }}>
      ...
      <div style={{ background: colors.dark, color: '#fff', padding: '0 24px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontWeight: 700, fontSize: 16 }}>
          ...
          <span>Customer Advocacy App</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {lastSynced && !refreshing && (
            <span style={{ fontSize: 11, color: '#666' }}>
              {quarterSubmissions.filter(r => !r._pending).length} submissions
            </span>
          )}
          <div style={{ display: 'flex', background: '#222', borderRadius: 8, padding: 2 }}>
            {QUARTERS.map(q => (
              <button
                key={q}
                onClick={() => setQuarter(q)}
                style={{
                  background: quarter === q ? '#fff' : 'none',
                  color: quarter === q ? colors.dark : '#aaa',
                  border: 'none', borderRadius: 6, padding: '4px 10px',
                  fontSize: 12, fontWeight: quarter === q ? 700 : 500, cursor: 'pointer',
                }}
              >
                {q}
              </button>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px' }}>
        ...
        {loading ? (
          ...
          <>
            ...
            {tab === 'dashboard' && (
              <Dashboard submissions={quarterSubmissions} quarter={quarter} />
            )}

            {tab === 'leaderboard' && (
              <Leaderboard submissions={quarterSubmissions} quarter={quarter} />
            )}

            {tab === 'log' && (
              <SubmissionLog
                submissions={quarterSubmissions}
