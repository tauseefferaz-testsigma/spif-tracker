import { currentWeekNumber, PROGRAM_WEEKS, getPaceStatus } from '../types/index.js';
...
export function buildTeamSummaryMessage(submissions) {
  ...
  const lbLines = topRows.map((c, i) => {
    ...
    return `${medals[i]} ${shortName(c.name).padEnd(9)} — ${c.pts} pts  ${pace ? paceEmoji(pace) : '⚪'}`;
  }).join('\n');
...
export function buildCsmSnapshotMessage(submissions) {
  const week = currentWeekNumber();
...
export async function sendSlackMessage(submissions, messageType = 'team') {
  const text = messageType === 'csm' 
    ? buildCsmSnapshotMessage(submissions)
    : buildTeamSummaryMessage(submissions);
