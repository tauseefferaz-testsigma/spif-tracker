...
export async function sendSnapshotImageToSlack(submissions) {
  try {
    ...
    const dashboardElement = document.querySelector('[data-dashboard-snapshot]') || 
        ...
        reader.onload = async () => {
          try {
            ...
            const response = await fetch(SLACK_API_URL, {
              ...
              headers: { "Content-Type": "application/json; charset=utf-8" },
              body: JSON.stringify({
                text: `📸 Weekly Snapshot - ${dateStr} | Week ${week}`,
                image_base64: base64,
                image_type: "png",
              }),
            });
    ...
    // Fallback: send text message if image capture fails
    console.warn("Image capture failed, sending text snapshot instead:", error);
    return sendSlackUpdate(submissions, "team");
  }
}
