...
import { sendSlackUpdate, sendSnapshotImageToSlack, isSlackConfigured } from "../lib/slack.js";
...
export default function Dashboard({ submissions }) {
  ...
  async function handleSendSlack(format = "team") {
    ...
    try {
      ...
      let formatLabel = "Weekly snapshot";
  ...
  async function handleSendSnapshotImage(submissions) {
    setSlacking(true);
    setSlackMsg(null);
    try {
      const result = await sendSnapshotImageToSlack(submissions);
      setSlackMsg({ ok: true, text: result.message || "Snapshot sent to Slack! 📸" });
    } catch (err) {
      setSlackMsg({ ok: false, text: err.message });
    } finally {
      setSlacking(false);
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {isSlackConfigured() && (
            <>
              <Button variant="secondary" onClick={() => handleSendSlack("team")} disabled={slacking}>
                {slacking ? "Sending…" : "💬 Weekly Snapshot"}
              </Button>
          ...
          <Button variant="secondary" onClick={() => handleSendSnapshotImage(submissions)} disabled={slacking}>
