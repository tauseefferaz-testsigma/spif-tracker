...
export default function App() {
  ...
  const { submissions, loading, refreshing, error: dataError, lastSynced, reload, create } = useSubmissions();
      ...
      <div style={{ minHeight: '100vh', background: colors.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div style={{ maxWidth: 500, background: '#fff', border: `1px solid ${colors.border}`, borderRadius: 16, padding: 36 }}>
          ...
          <ol style={{ color: colors.mid, fontSize: 14, paddingLeft: 20, lineHeight: 2 }}>
            <li>Deploy Google Apps Script backend</li>
    ...
    <div style={{ minHeight: '100vh', background: colors.bg }}>
      <div style={{ background: colors.dark, color: '#fff', padding: '0 24px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {refreshing && <span style={{ fontSize: 11, color: '#aaa' }}>syncing…</span>}
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px' }}>
        {dataError && (
          <div style={{
            ...
            <button onClick={reload} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#7f1d1d', fontWeight: 700, fontSize: 13 }}>
              Retry
            </button>
          </div>
        )}
          ...
          <>
            {tab === 'submit' && (
              <div style={{ maxWidth: 560 }}>
                <SubmissionForm onSubmit={handleFormSubmit} disabled={refreshing} />
              </div>
            )}
            {tab === 'dashboard' && <Dashboard submissions={submissions} />}
            {tab === 'leaderboard' && <Leaderboard submissions={submissions} />}
          </>
        )}
      </div>
