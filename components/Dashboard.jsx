...
// ─── MAIN DASHBOARD ──────────────────────────────────────────────────────────
export default function Dashboard({ submissions, quarter = CURRENT_QUARTER }) {
  const summary  = useMemo(() => buildTeamSummary(submissions, quarter),    [submissions, quarter]);
  const csmStats = useMemo(() => buildCsmStats(submissions, quarter),       [submissions, quarter]);
  const actBreak = useMemo(() => buildActivityBreakdown(submissions, quarter), [submissions, quarter]);
  ...
  async function handleSendSlack(format = "team") {
    setSlacking(true);
    setSlackMsg(null);
    try {
      const result = await sendSlackUpdate(submissions, format, quarter);
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: colors.dark, margin: 0 }}>📊 {quarter} Dashboard</h2>
          <div style={{ fontSize: 12, color: colors.muted, marginTop: 3 }}>
            Week {week} of {PROGRAM_WEEKS} · Program starts {new Date("2026-05-18").toLocaleDateString("en-US", { month: "short", day: "numeric" })}
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {isSlackConfigured() && (
            <>
              <Button variant="secondary" onClick={() => handleSendSlack("team")} disabled={slacking}>
                {slacking ? "Sending…" : "💬 Send to Slack"}
              </Button>
              <Button variant="secondary" onClick={() => handleSendSlack("csm")} disabled={slacking}>
                {slacking ? "Sending…" : "📋 CSM Snapshot"}
              </Button>
            </>
          )}
          <Button variant="secondary" onClick={() => exportReport(submissions, quarter)}>
            ⬇ Download PDF
          </Button>
        </div>
      </div>
      ...
      <Card style={{ marginBottom: 20, padding: "14px 20px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: colors.mid }}>{quarter} Sprint Progress</span>
      ...
      {/* ── PER-CSM TARGET GRID ──────────────────────────────────────────────── */}
      <SectionTitle>Individual Progress — {quarter} Targets</SectionTitle>
      ...
      {withoutTargets.length > 0 && (
        <>
          <SectionTitle>Team Members — No {quarter} Target Assigned</SectionTitle>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12, marginBottom: 20 }}>
